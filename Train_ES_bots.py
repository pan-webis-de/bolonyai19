...
# lists: dev
r = open("es/truth-dev.txt", "r") #path for es truth-dev file
dev = r.read().split("\n")
dev_l = [] #id
dev_b = [] #bot/human
dev_n = [] #male/female
...
# lists: train
r = open("es/truth-train.txt", "r") #path for es truth-train file
train = r.read().split("\n")
train_l = [] #id
train_b = [] #bot
train_n = [] #gender
...
pathlist = Path("pan19-author-profiling-training-2019-02-18/es").glob('**/*.xml')
#path for es training set
...
def SentiScore(text):
    sentiscore = senti_clf.predict(text)
    return sentiscore
...
def kukactot(lista):
    ...
    for j in range(0,int((len(lista)/100))):
        kukac_ossz.append(sum(lista[j*100:(j+1)*100])) #lista: amiben eredetileg megnéztük, hogy mennyi kukac van
    return kukac_ossz

def calls(text): #ki jön a @ után, ezt végig kell futtatni tweetenként a többivel együtt, 
...
def friends_ratio(nom,denom):
    ...
    for j in range(0,len(denom)):
        if denom[j]!=0:
            ratio.append(len(nom[j])/denom[j]) #nom: amibe elmentjük a calls eredményét
...
def RT_author(text): #ki volt az eredeti tweet szerzője, ezt végig kell futtatni tweetenként a többivel együtt, 
...
def RT_ratio(nom,denom):
    ...
    for j in range(0,len(denom)):
        if denom[j] !=0:
            ratio.append(len(nom[j])/denom[j])#nom: amibe kimentjük az RT_author eredményét, 
...
lexdiv_dev=[] #lexikaidiverzitás(RootTTR)
noun_dev=[] #főnevekarányaatweetben
verb_dev=[] #igékarányaatweetben
adj_dev=[] #melléknevekarányaatweetben
stoparany_dev=[] #stopszavakarányaatweetben
sentiscore_dev=[] #szentimentszkóratweetre
count_emojis_dev=[] #hányemojivanatweetben
emoji_hanyfele_dev=[] #hányféleemojivanatweetben
emoji_arany_dev=[] #emojikarányaatweetben
misspelled_dev=[] #hányszóvanrosszulírvaatweetben
kukacok_dev=[]
friends_dev=[]
RT_dev=[]
RT_authors_dev=[]
...
for l in dev_x_raw[100:]:
    ...
    for tweet in l:
        try:
            ...
            szam_l_dev.append(gyakorisagok(tweet)[2])
            pont_l_dev.append(gyakorisagok(tweet)[3])
            vesz_l_dev.append(gyakorisagok(tweet)[4])
            irj_l_dev.append(gyakorisagok(tweet)[5])
            szo_l_dev.append(gyakorisagok(tweet)[6])
            bet_l_dev.append(gyakorisagok(tweet)[7])
            flood_l_dev.append(gyakorisagok(tweet)[8])
            apar_l_dev.append(gyakorisagok(tweet)[9])
            nagybar_l_dev.append(gyakorisagok(tweet)[10])
            szamar_l_dev.append(gyakorisagok(tweet)[11])
            modath_l_dev.append(gyakorisagok(tweet)[12])
            veszar_l_dev.append(gyakorisagok(tweet)[13])
            irjar_l_dev.append(gyakorisagok(tweet)[14])
            szohossz_l_dev.append(gyakorisagok(tweet)[15])
            floodar_l_dev.append(gyakorisagok(tweet)[16])
        except Exception as e:
            print(e)
            error_dev += 1
    except:
...
classifier2 = LogisticRegression(C=.001,fit_intercept=False)
...
array = df_agg_train_es.values
x_train = array[:,1:df_agg_train_es.shape[1]]
y_train = array[:,0]

array = df_agg_test_es.values
x_test = array[:,1:df_agg_test_es.shape[1]]
y_test = array[:,0]

classifier_agg = LogisticRegression(C=.001,fit_intercept =True,solver = 'lbfgs')
